...
const valid: SourceContent = { id: crypto.randomUUID(), categoryId: 'economy', categoryName: '경제', contentGroup: 'news', wrapperClass: 'daily-brief-note news-briefing economy', slugPattern: 'economy-briefing-YYYY-MM-DD', seriesNo: null, briefingDate: '2026-07-18', publishedOn: null, contentStatus: 'ready', updatedAt: '2026-07-18T00:00:00Z', representativeTitle: '대표 제목', metaDescription: meta, htmlBody: `<div class="daily-brief-note news-briefing economy"><h1>대표 제목</h1><a href="#sources">출처</a><section id="sources"><h2>출처</h2><ul><li><a href="${sourceUrl}">Source</a></li></ul></section></div>`, slug: 'economy-briefing-2026-07-18', sources: [{ name: 'Example', title: 'Economy source', url: sourceUrl, checkedPoint: '테스트 출처 확인' }], tags: ['A','B','C','D','E'].map((name) => ({ id: crypto.randomUUID(), name, normalizedName: name.toLowerCase() })) }
...
describe('publication payload builder', () => {
  ...
  it('ready가 아닌 콘텐츠를 차단한다', async () => expect((await buildPayload({ ...valid, contentStatus: 'draft' }, [], [])).blockers.some((item) => item.code === 'CONTENT_NOT_READY')).toBe(true))
  ...
  it('출처 레코드가 없으면 차단한다', async () => expect((await buildPayload({ ...valid, sources: [] }, [], [])).blockers.some((item) => item.code === 'SOURCE_MISSING')).toBe(true))
  it('저장된 출처 URL이 section#sources에 없으면 차단한다', async () => expect((await buildPayload({ ...valid, htmlBody: valid.htmlBody!.replace(sourceUrl, 'https://source.example.test/other') }, [], [])).blockers.some((item) => item.code === 'SOURCE_HTML_MISMATCH')).toBe(true))
