-- Phase 4C-0: align current category slug rules without rewriting published posts.

update public.categories
set slug_pattern = 'technology-briefing-YYYY-MM-DD',
    updated_at = statement_timestamp()
where id = 'technology'
  and code = 'TEC'
  and content_group = 'news'
  and slug_pattern = 'science-tech-briefing-YYYY-MM-DD';

update public.categories
set slug_pattern = 'climate-energy-briefing-YYYY-MM-DD',
    updated_at = statement_timestamp()
where id = 'climate-energy'
  and code = 'ENV'
  and content_group = 'news'
  and slug_pattern = 'environment-briefing-YYYY-MM-DD';

update public.categories
set slug_pattern = 'cctv-chinese-news-###',
    updated_at = statement_timestamp()
where id = 'chinese-study'
  and code = 'CHINESE'
  and content_group = 'chinese'
  and slug_pattern = 'cctv-chinese-news-study-###';
...
begin
  if exists (
    ...
    where (id = 'technology' and code = 'TEC' and content_group = 'news'
           and slug_pattern <> 'technology-briefing-YYYY-MM-DD')
       or (id = 'climate-energy' and code = 'ENV' and content_group = 'news'
           and slug_pattern <> 'climate-energy-briefing-YYYY-MM-DD')
       or (id = 'chinese-study' and code = 'CHINESE' and content_group = 'chinese'
           and slug_pattern <> 'cctv-chinese-news-###')
  ) then
    raise exception 'CATEGORY_SLUG_PATTERN_CONFLICT';
  end if;
end;
$$;

create or replace function public.import_content_post(p_item jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
declare
  ...
  validation_mode_value text := coalesce(nullif(btrim(p_item ->> 'validation_mode'), ''), 'strict');
...
begin
  ...
  select key into unknown_key
   ...
   where key <> all(array[
     'validation_mode', 'category_id', 'title', 'summary', 'slug', 'status',
     'briefing_date', 'published_on', 'published_at', 'display_id', 'series_no',
     'wordpress_url', 'html_body', 'seo', 'image', 'tags', 'sources', 'metadata'
   ])
   limit 1;
  if unknown_key is not null or validation_mode_value not in ('strict', 'legacy') then
    raise exception 'IMPORT_VALIDATION_FAILED' using errcode = '22023';
  end if;
  ...
  if category_row.content_group = 'news' then
    ...
    expected_display_id := case when category_row.display_id_pattern is null then null
        ...
        category_row.display_id_pattern,
        '###',
        lpad(series_no_value::text, greatest(3, char_length(series_no_value::text)), '0')
      ) end;
  end if;
  ...
  expected_slug := replace(
    expected_slug,
    '###',
    lpad(
      coalesce(series_no_value, 0)::text,
      greatest(3, char_length(coalesce(series_no_value, 0)::text)),
      '0'
    )
  );
  if validation_mode_value = 'strict' and slug_value <> expected_slug then
    raise exception 'IMPORT_VALIDATION_FAILED' using errcode = '23514';
  end if;
...
comment on function public.import_content_post(jsonb) is
  'Atomically imports one normalized content item; strict inputs use current category slug rules and explicit legacy inputs preserve historical slugs.';
