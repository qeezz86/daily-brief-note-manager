...
| Largest chunk | 198,470 B | 50,700 B |
...
Report schema version 1에는 manifest의 저장소 상대 경로, config/baseline version, metric과 limit, violation/warning, size 내림차순 chunk, route/feature closure asset과 pass 상태가 포함된다. 생성 report에는 전체 source code, 환경변수, 사용자명, 사용자 home 또는 Windows 절대 경로를 넣지 않는다. Hashed output filename은 실행 결과 추적을 위해 report에만 포함할 수 있다.
