...
구조화 블록에 없는 필드는 `[WORDPRESS_HTML]`의 DOM 구조에서 추출하고 미리보기에서 사용자가 보완한다. MVP의 구조화 형식은 위에 정의된 키만 필수 지원 대상으로 삼으며, 출처·뉴스 추적·카테고리별 메타데이터를 위한 임의의 추가 키를 만들지 않는다.
...
- `published_at`처럼 시각 필드에 날짜-only 값만 들어오면 임의의 시각을 만들지 않고 후보·경고로 표시하여 사용자가 시각을 보완하게 한다.
...
1. `[SEO].tags`의 `|` 문자가 태그 자체에 포함될 때의 escape 또는 금지 규칙이 없다.
2. 비구조화 응답의 표준 제목 문자열 목록과 각 section 경계 규칙이 명시되지 않았다.
3. strict validation의 기본 활성화 여부가 명시되지 않았다.
4. 내부 링크 `<a>`의 구체적인 URL 허용 범위와 오류·경고 구분이 명시되지 않았다.
5. 일반 콘텐츠의 `published_at`에 시각이 없을 때 반드시 입력을 요구할지 nullable로 저장할지 확정이 필요하다.
6. JSON `update`가 기존 하위 관계를 교체하는지 병합하는지, 내부 UUID를 보존할지 재매핑할지 확정이 필요하다.
7. JSON의 정확한 최상위 필드와 테이블별 배열 구조, 관계 복원 순서가 아직 정해지지 않았다.
8. 비구조화 응답에서 최우선으로 탐지할 JSON 블록의 객체 키와 허용 중첩 구조는 아직 정해지지 않았다.
