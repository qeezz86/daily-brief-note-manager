...
WordPress가 유효한 draft 응답을 반환한 뒤 `succeeded` audit transition이 실패하면 Function은 같은 `executing` row를 `AUDIT_RECORD_FAILED`의 `uncertain`으로 바꾸는 best-effort transition을 한 번 수행한다. 첫 transition이 실제 commit된 뒤 응답만 유실됐거나 DB 자체가 unavailable이면 fallback도 실패할 수 있으므로 client에는 항상 `WORDPRESS_DRAFT_RESULT_UNCERTAIN`을 반환하고 수동 reconciliation을 요구한다. 기존 execution lock 때문에 WordPress POST를 다시 수행하지 않는다.
