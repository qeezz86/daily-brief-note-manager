...
- 콘텐츠 편집의 출처 일시는 일반 URL이면 서울, 공식 CCTV 개별 원문 URL이면 상하이 기준으로 표시·입력한다. 중국어 원문 일시는 상하이 기준이며 입력란에 시간대를 표시한다. 저장 요청에는 명시적 시간대가 포함된 시각을 전달하여 브라우저·DB 기본 시간대에 의존하지 않는다. 시각을 수정하지 않으면 기존 초·소수초 정밀도를 보존한다. 과거에 잘못 저장된 시각은 원문 확인 없이 일괄 보정하지 않는다.
...
### Phase 5O-A 콘텐츠 영구 삭제

- 콘텐츠 상세에서 제목이 포함된 확인창을 거쳐 Content Manager의 로컬 콘텐츠를 영구 삭제할 수 있다. 삭제한 콘텐츠는 복구할 수 없다.
- Import 이력 또는 WordPress 발행 시도 이력이 연결된 콘텐츠는 이력 보존을 위해 삭제할 수 없다. 두 조회가 모두 성공하고 참조가 없을 때만 허용하며, 확인 중이거나 조회에 실패하면 삭제를 막는다.
- Phase 5O-A는 기존 데이터베이스 구조와 보호 규칙을 그대로 사용하며 스키마·마이그레이션·RLS 정책·FK 정의를 변경하지 않는다. 콘텐츠 삭제는 기존 소유자 범위 DELETE 정책을 따르고 FK RESTRICT 제약을 최종 DB 보호 장치로 유지한다.
- WordPress 원격 게시물은 삭제하지 않는다. 과거 백업, 생성된 프롬프트와 스냅샷도 정리하지 않는다.
- 보관과 삭제는 별도 동작이다. 보관된 콘텐츠도 다른 삭제 조건을 충족하면 영구 삭제할 수 있다.
- 삭제 성공 시 콘텐츠 목록으로 이동해 “콘텐츠를 삭제했습니다.”를 표시한다. 실패 시 상세와 콘텐츠를 유지한다.
- Phase 5O-A에는 목록에서의 삭제와 일괄 삭제를 포함하지 않는다.
...
현재 목록은 카테고리·상태·제목/slug 검색 조건을 서버에서 적용하고, 수정일 내림차순과 내부 ID 내림차순으로 20개씩 조회한다. 검색은 앞뒤 공백을 제거한 대소문자 무관 부분 일치이며 특수문자도 입력한 문자 그대로 취급한다. 조건이 없으면 전체 개수, 조건이 있으면 일치하는 결과의 정확한 개수와 현재 표시 범위를 보여 준다. 이전·다음 페이지로 이동하며 조건 변경과 검색 초기화는 첫 페이지로 돌아간다. 조회 중 삭제로 현재 페이지가 없어지면 같은 조건의 첫 페이지를 다시 조회한다. 기존 소유권 RLS를 그대로 적용하고 1,000행 응답 제한으로 검색 대상이 잘리지 않도록 한다.
