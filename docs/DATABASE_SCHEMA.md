...
`save_chatgpt_paste_post(p_item jsonb) returns jsonb`는 Phase 5G의 구조화 ChatGPT 붙여넣기 한 건 저장 전용 `SECURITY DEFINER` 함수다. 빈 `search_path`를 고정하고 authenticated caller만 실행할 수 있으며 owner는 오직 `auth.uid()`에서 얻는다. 입력은 `content`, `seo`, `image`, `sources`, `html_body`의 exact JSON shape와 각 nested key allowlist를 검사하고 owner/auth/session/raw paste/provenance/NEWS_TRACKING/unknown key를 거부한다. source `sort_order`는 caller 값이 아니라 배열 순서에서 0부터 생성한다.

함수는 기존 `import_content_post(jsonb)`를 같은 transaction 안에서 strict draft aggregate 저장에 재사용한다. 반환된 `postId`가 현재 authenticated owner에게 방금 생성된 `json_import` 행인지 확인한 뒤 그 정확한 행의 `source_import_type`만 서버에서 `chatgpt_paste`로 변경한다. 이 확인이나 provenance 변경, tag·source·SEO 등 aggregate 하위 저장이 실패하면 전체 wrapper transaction이 rollback되어 post와 orphan source가 남지 않는다. upsert, 기존 post overwrite, historical row rewrite, news update/followup 저장은 하지 않으며 기존 `import_content_post`의 `json_import` 동작과 default를 변경하지 않는다.
