...
이 문서는 Phase 5C 코드를 원격 Supabase, 실제 프런트엔드와 실제 WordPress에 적용하는 Phase 5C-R2 실행 절차다. Phase 5C-R2A read-only inspection에서 원격 DB가 fresh project이고 pending migration이 마지막 3개가 아니라 전체 22개임을 확인했다. 이 문서의 원격 명령과 WordPress 요청은 별도 실행 승인 전에는 실행하지 않는다.
...
| 단위 | 파일·대상 | 분류 | 비고 |
|---|---|---|---|
| DB baseline | `config/supabase-fresh-project-baseline.json`의 22개 migration | fresh remote DB deployment | fresh project에서만 exact order 전체 적용 |
| DB incremental | 마지막 WordPress migration 3개 | existing remote DB deployment | 앞 19개 history/schema가 일치할 때만 적용 |
...
- `FRESH_PROJECT_BASELINE_REQUIRED`: migration history 0, application table/function 0, local migration 22개 전부 pending, unexpected/remote-only object 0.
- `EXISTING_PROJECT_INCREMENTAL_READY`: 앞 19개 migration과 core schema가 일치하고 마지막 3개만 pending.
...
현재 확인 상태는 첫 번째이며 실제 원격 write, Function deploy, secret 변경, WordPress 요청은 모두 0건이다. 이전 Gate 2의 “마지막 3개만 pending” 판단은 실제 `db push --dry-run` evidence와 달랐고 Gate 4는 해당 dry-run에서 안전하게 중단됐다. Migration repair가 아니라 fresh baseline이 필요하다.
...
3. pending이 manifest의 22개와 순서까지 정확히 일치하는지 확인한다.
...
7. 전체 baseline 22개와 category seed 적용에 대한 별도 명시적 승인을 받는다.
8. 승인된 22개 migration 전체를 manifest 순서로 적용한다.
9. 승인된 category seed를 한 번 적용한다.
10. migration history가 정확히 22개인지 확인한다.
...
Fresh에서 마지막 3개만 선택 적용하거나 seed를 생략하는 계획은 실패다. 상세 순서와 read-only SQL은 `docs/SUPABASE_FRESH_PROJECT_BASELINE.md`를 따른다.

## 6C. Existing Project Incremental Path

기존 경로는 삭제하지 않는다. 앞 19개 migration history와 core schema가 정확히 일치하고 마지막 WordPress 3개만 pending일 때에만 다음 세 migration을 순서대로 적용한다.
...
Incremental 경로에서는 category seed를 기본적으로 재실행하지 않는다. 전체 22개를 다시 적용하는 계획도 실패다. DB 적용에는 별도 명시적 승인이 필요하다.
...
승인 요청에는 exact commit, clean tree, backup, sanitized classification, 22개 manifest, seed whitelist, local reset/lint/pgTAP, Vitest/Deno/smoke/E2E/lint/build/bundle 결과와 예상 원격 객체를 포함한다. 이 승인은 DB baseline과 seed만 허용하며 Function, secret, frontend, WordPress draft를 허용하지 않는다.

## 6G. Fresh Baseline Post-Deployment Verification

Migration history 22개, expected public table, RLS/policy, RPC security/search path/execute, unique index와 partial execution guard, category 8개, user-owned content 0, WordPress mapping/attempt 0을 read-only SQL로 확인한다. 하나라도 다르면 WordPress 단계로 진행하지 않는다.
...
Dry run의 pending 목록은 먼저 6A의 모드로 분류한다. Fresh이면 manifest의 22개 전체, existing이면 승인된 Phase 5B/5C 마지막 3개와 정확히 일치해야 한다. 그 밖의 오래된 일부 migration, 알 수 없는 remote-only version, unexpected object 또는 history mismatch가 보이면 push하지 않는다.
...
이 절은 `EXISTING_PROJECT_INCREMENTAL_READY`에만 적용한다. Fresh project는 6B와 baseline manifest를 따른다. 최종 DB 변경 승인을 받은 뒤에만 실행한다.
...
- 선택된 모드의 remote migration 적용 완료(fresh 22개 또는 existing 마지막 3개), table/RLS/RPC/index/constraint 검증 완료
