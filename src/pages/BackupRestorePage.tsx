...
import { loadRestorePlanModule, type RestorePlanModule } from '../features/backups/restorePlan.loader'
import type { RestorePlan, RestorePolicies } from '../features/backups/restorePlan.types'
import { loadRestoreValidationModule, type RestoreValidationModule } from '../features/backups/restoreValidation.loader'
...
export function BackupRestorePageContent({
  client = supabase,
  userId = '',
  loadValidationModule = loadRestoreValidationModule,
  loadPlanModule = loadRestorePlanModule,
}: {
  client?: DatabaseClient | null
  userId?: string
  loadValidationModule?: () => Promise<RestoreValidationModule>
  loadPlanModule?: () => Promise<RestorePlanModule>
}) {
  const [text, setText] = useState('')
  const [file, setFile] = useState<File | null>(null)
  const [busy, setBusy] = useState(false)
  const [validationModuleLoading, setValidationModuleLoading] = useState(false)
  ...
  const [planModuleLoading, setPlanModuleLoading] = useState(false)
  const [planModuleLoadFailed, setPlanModuleLoadFailed] = useState(false)
  ...
  async function run() {
    ...
    setValidationModuleLoading(true)
    let validationModule: RestoreValidationModule
    try {
      validationModule = await loadValidationModule()
      if (!mountedRef.current) return
    } catch {
      if (mountedRef.current) {
        setError('RESTORE_MODULE_LOAD_FAILED 필요한 기능을 불러오지 못했습니다. 네트워크 상태를 확인한 뒤 다시 시도하세요.')
        setBusy(false)
      }
      return
    } finally {
      if (mountedRef.current) setValidationModuleLoading(false)
    }
    try {
      const parsed = file ? await validationModule.parseBackupFile(file) : validationModule.parseBackupText(text)
      if (!mountedRef.current) return
      ...
      if (!mountedRef.current) return
      const local = await validationModule.validateBackupForRestore(parsed.value, { currentCategories: categories, currentAppVersion: appVersion() })
      if (!mountedRef.current) return
      ...
      if (!mountedRef.current) return
      const final = await validationModule.validateBackupForRestore(parsed.value, { currentCategories: categories, lookup, currentAppVersion: appVersion() })
      if (!mountedRef.current) return
      setResult(final.result)
      if (final.bundle) setPlanContext({ bundle: final.bundle, categories, lookup })
    } catch {
      if (mountedRef.current) setError('복원 Dry Run 검사 중 현재 설정 또는 DB 충돌 후보를 확인하지 못했습니다. 데이터는 변경되지 않았습니다.')
    } finally { if (mountedRef.current) setBusy(false) }
  }

  async function createPlan(nextPolicies = policies) {
    if (!planContext || !client || planBusy) return
    setPlanBusy(true); setError(null); setPlanModuleLoading(true); setPlanModuleLoadFailed(false)
    let planModule: RestorePlanModule
    try {
      planModule = await loadPlanModule()
      if (!mountedRef.current) return
    } catch {
      if (mountedRef.current) {
        setError('RESTORE_MODULE_LOAD_FAILED 필요한 기능을 불러오지 못했습니다. 네트워크 상태를 확인한 뒤 다시 시도하세요.')
        setPlanModuleLoadFailed(true)
        setPlanBusy(false)
      }
      return
    } finally {
      if (mountedRef.current) setPlanModuleLoading(false)
    }
    try {
      const provisional = await planModule.buildRestorePlan({ ...planContext, currentCategories: planContext.categories, policies: nextPolicies })
      if (!mountedRef.current) return
  ...
  return (
    <section className="content-page backup-page" aria-labelledby="backup-restore-page-title" aria-busy={validationModuleLoading || planModuleLoading}>
      ...
      {step === 'dry-run' ? <>
        ...
        {validationModuleLoading ? <section className="backup-panel" role="status"><h2>복원 검증 도구 로딩</h2><p>복원 검증 도구를 불러오는 중입니다.</p></section> : busy ? <section className="backup-panel" aria-live="polite"><h2>검사 진행 상태</h2><p>checksum과 local schema를 확인한 뒤 현재 DB를 100개 단위로 조회하고 있습니다.</p></section> : null}
        ...
        <div className="backup-actions"><button type="button" className="secondary-button" disabled={planBusy} onClick={() => setStep('dry-run')}>Dry Run으로 돌아가기</button><button type="button" className="primary-button" disabled={planBusy || (!planStale && !planModuleLoadFailed)} onClick={() => void createPlan()}>{planModuleLoading ? '복원 계획 도구 불러오는 중' : planBusy ? '계획 생성 중' : planModuleLoadFailed ? '계획 생성 다시 시도' : '계획 다시 생성'}</button></div>
        {planModuleLoading ? <section className="backup-panel" role="status"><h2>복원 계획 도구 로딩</h2><p>복원 계획 도구를 불러오는 중입니다.</p></section> : planBusy && !plan ? <section className="backup-panel" aria-live="polite"><h2>복원 계획 생성 중</h2><p>정책, UUID v5 remap target, 관계 graph와 fingerprint를 검증하고 있습니다.</p></section> : null}
        {error ? <p className="form-alert" role="alert">{error}</p> : null}
