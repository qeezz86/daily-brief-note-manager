...
import { loadImportAnalysisModule, type ImportAnalysisModule } from '../features/imports/importAnalysis.loader'
...
const NewsResponseImportWorkflow = lazy(() =>
  import('../features/imports/NewsResponseImportWorkflow').then((module) => ({
    default: module.NewsResponseImportWorkflow,
  })),
)
...
export function ImportPageContent({
  client = supabase,
  userId = '',
  loadAnalysisModule = loadImportAnalysisModule,
}: {
  client?: DatabaseClient | null
  userId?: string
  loadAnalysisModule?: () => Promise<ImportAnalysisModule>
}) {
  ...
  const [importMode, setImportMode] = useState<'json' | 'chatgpt-paste' | 'news-response' | 'non-news-response' | 'wordpress-html'>('json')
  ...
  function resetResult() {
    if (busy) return
    setResult(null); setParsedInput(null); setMetadata(null); setSelected(new Set()); setApprovedWarnings(new Set()); setMessage(null); setCopyMessage(null); setValidatedCategorySignature(null); setModuleLoadError(false)
  }

  async function validate(text: string, inputMetadata: ImportInputMetadata) {
    if (busy) return
    setBusy(true); setModuleLoading(true); setModuleLoadError(false); setCopyMessage(null); setMessage(null)
    let analysisModule: ImportAnalysisModule
    try {
      analysisModule = await loadAnalysisModule()
    ...
    try {
      const input = analysisModule.parseImportJsonText(text)
      const lookup = client ? await getImportDuplicateReferenceData(client, analysisModule.collectImportDuplicateCandidates(input)) : { databaseCheck: 'unavailable' as const, referenceData: emptyDuplicateReferenceData }
      if (!mountedRef.current) return
      const nextResult = analysisModule.validateImportBundle(input, { categories, ...lookup.referenceData }, lookup.databaseCheck)
      ...
      if (mountedRef.current) {
        const inputError = error instanceof analysisModule.ImportInputError ? error : new analysisModule.ImportInputError('BUNDLE_VALIDATION_FAILED', 'JSON을 검증하지 못했습니다.')
        setResult(analysisModule.importInputErrorResult(inputError)); setParsedInput(null); setSelected(new Set()); setApprovedWarnings(new Set()); setMetadata(inputMetadata)
      }
  ...
  async function createJob() {
    ...
    setBusy(true); setModuleLoading(true); setModuleLoadError(false); setMessage(null); setCopyMessage(null)
    let analysisModule: ImportAnalysisModule
    try {
      analysisModule = await loadAnalysisModule()
      if (!mountedRef.current) return
    ...
    setMessage('Import 직전 DB 중복을 다시 확인하고 있습니다.')
    try {
      ...
      const lookup = await getImportDuplicateReferenceData(client, analysisModule.collectImportDuplicateCandidates({ ...root, posts: selectedRaw }))
      if (!mountedRef.current) return
      ...
      const fresh = analysisModule.validateImportBundle({ ...root, posts: selectedRaw }, { categories, ...lookup.referenceData }, 'complete')
      ...
      const prepared = await analysisModule.prepareImportJob(client, {
    ...
    } catch { if (mountedRef.current) setMessage('Import 작업 준비에 실패했습니다. 같은 입력으로 다시 시도하면 등록된 chunk부터 이어집니다.') } finally { if (mountedRef.current) setBusy(false) }
  }
  ...
  return <section className="content-page import-page" aria-labelledby="import-page-title" aria-busy={moduleLoading}>
    ...
    <fieldset className="import-mode-selector">
      ...
      <label><input type="radio" name="import-mode" value="news-response" checked={importMode === 'news-response'} onChange={() => setImportMode('news-response')} /> 뉴스 일반 응답 붙여넣기</label>
    ...
    </Suspense> : importMode === 'news-response' ? <Suspense fallback={<div className="content-state" role="status">뉴스 응답 가져오기 도구를 불러오는 중입니다.</div>}>
      {categoriesQuery.isPending ? <div className="content-state" role="status">뉴스 카테고리 설정을 불러오고 있습니다.</div> : null}
      {categoriesQuery.isError ? <div className="content-state content-state--error" role="alert">뉴스 카테고리 설정을 불러오지 못했습니다.</div> : null}
      <NewsResponseImportWorkflow client={client} categories={categories} />
