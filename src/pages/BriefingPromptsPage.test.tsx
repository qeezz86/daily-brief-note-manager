...
describe('BriefingPromptsPage', () => {
  ...
  it('defaults to standard mode, recent 5 posts and a 90-day lookback', async () => { renderPage(); expect(await screen.findByLabelText('프롬프트 모드')).toHaveValue('standard'); expect(screen.getByLabelText('최근 글 수')).toHaveValue('5'); expect(screen.getByLabelText('종료 뉴스 조회 기간')).toHaveValue(90) })
  it('offers only 5, 10 and 15 recent-post options', async () => {
    renderPage()
    await screen.findByLabelText('최근 글 수')
    expect(screen.getAllByRole('option', { name: /최근 글 \d+개/ }).map((option) => (option as HTMLOptionElement).value)).toEqual(['5', '10', '15'])
  })
  it('shows the selected category template settings and version', async () => { renderPage(); expect(await screen.findByText('경제 브리핑 작성 템플릿')).toBeInTheDocument(); expect(screen.getByText('daily-brief-note news-briefing economy')).toBeInTheDocument(); expect(screen.getByText(/#\d{4}-\d{2}-\d{2}-ECO/)).toBeInTheDocument(); expect(screen.getByText(/economy-briefing-\d{4}-\d{2}-\d{2}/)).toBeInTheDocument(); expect(screen.getByText('Template version').nextElementSibling).toHaveTextContent('v1') })
  it('blocks an unsupported active news category with an understandable error', async () => { const unsupported = [{ ...categories[0], id: 'unknown', name: '미지원 뉴스' }]; renderPage(createClient(context, null, unsupported).client); expect(await screen.findByText(/지원하지 않는 뉴스 카테고리입니다: unknown/)).toBeInTheDocument(); expect(screen.getByRole('button', { name: '프롬프트 생성' })).toBeDisabled() })
  it('blocks an incomplete category format', async () => { const invalid = [{ ...categories[0], wrapper_class: '' }]; renderPage(createClient(context, null, invalid).client); expect(await screen.findByText(/wrapper class가 설정되지 않았습니다/)).toBeInTheDocument(); expect(screen.getByRole('button', { name: '프롬프트 생성' })).toBeDisabled() })
  ...
  it.each([
    ...
    ['recent count', async (user: ReturnType<typeof userEvent.setup>) => user.selectOptions(screen.getByLabelText('최근 글 수'), '15')],
  ...
  it('keeps recent count when mode and category change and after generation', async () => {
    const user = userEvent.setup(); renderPage(); await screen.findByRole('option', { name: '경제' })
    await user.selectOptions(screen.getByLabelText('최근 글 수'), '15')
    await user.selectOptions(screen.getByLabelText('프롬프트 모드'), 'detailed')
    await user.selectOptions(screen.getByLabelText('뉴스 카테고리'), 'global')
    expect(screen.getByLabelText('최근 글 수')).toHaveValue('15')
    await user.click(screen.getByRole('button', { name: '프롬프트 생성' }))
    await screen.findByLabelText('복사용 프롬프트')
    expect(screen.getByLabelText('최근 글 수')).toHaveValue('15')
  })
  ...
  it('saves the exact preview text and validation summary snapshot', async () => { const user = userEvent.setup(); const { client, rpc } = createClient(); renderPage(client); await screen.findByRole('option', { name: '경제' }); await user.click(screen.getByRole('button', { name: '프롬프트 생성' })); const preview = await screen.findByLabelText('복사용 프롬프트') as HTMLTextAreaElement; await user.click(screen.getByRole('button', { name: '현재 프롬프트 저장' })); await waitFor(() => expect(rpc).toHaveBeenCalledWith('save_news_briefing_prompt_run', expect.objectContaining({ p_prompt_text: preview.value, p_context_snapshot: expect.objectContaining({ ...context, promptValidationVersion: 1, promptValidationSummary: expect.objectContaining({ status: 'valid', errorCount: 0 }) }) }))); expect(screen.getByText(/프롬프트 이력을 저장했습니다/)).toBeInTheDocument() })
  it('shows requested and actual counts and sends selected count 15 to save', async () => {
    const user = userEvent.setup(); const { client, rpc } = createClient(); renderPage(client); await screen.findByRole('option', { name: '경제' })
    await user.selectOptions(screen.getByLabelText('최근 글 수'), '15')
    await user.click(screen.getByRole('button', { name: '프롬프트 생성' }))
    expect(await screen.findByText('요청 15개 · 실제 사용 1개')).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: '현재 프롬프트 저장' }))
    await waitFor(() => expect(rpc).toHaveBeenCalledWith('save_news_briefing_prompt_run', expect.objectContaining({ p_requested_post_count: 15 })))
  })
  it('blocks save and prompt copy when context validation has errors', async () => {
    const user = userEvent.setup(); const invalid = { ...context, recentPosts: [context.recentPosts[0], context.recentPosts[0]], counts: { ...context.counts, recentPosts: 2, recentUpdates: 2 } }
    renderPage(createClient(invalid).client); await screen.findByRole('option', { name: '경제' }); await user.click(screen.getByRole('button', { name: '프롬프트 생성' }))
    expect(await screen.findByText('오류 있음')).toBeInTheDocument(); expect(screen.getByRole('button', { name: '현재 프롬프트 저장' })).toBeDisabled(); expect(screen.getByRole('button', { name: '프롬프트 복사' })).toBeDisabled()
  })
  it('allows save when validation has only data-availability warnings', async () => {
    const user = userEvent.setup(); const { client, rpc } = createClient({ ...context, recentPosts: [], openTopics: [], pendingFollowups: [], recentClosedTopics: [], counts: { recentPosts: 0, recentUpdates: 0, openTopics: 0, pendingFollowups: 0, overdueFollowups: 0, recentClosedTopics: 0 } })
    renderPage(client); await screen.findByRole('option', { name: '경제' }); await user.click(screen.getByRole('button', { name: '프롬프트 생성' }))
    expect(await screen.findByText('경고 있음')).toBeInTheDocument(); expect(screen.getByRole('button', { name: '현재 프롬프트 저장' })).toBeEnabled(); await user.click(screen.getByRole('button', { name: '현재 프롬프트 저장' })); await waitFor(() => expect(rpc).toHaveBeenCalledWith('save_news_briefing_prompt_run', expect.objectContaining({ p_context_snapshot: expect.objectContaining({ promptValidationSummary: expect.objectContaining({ status: 'warning' }) }) })))
  })
