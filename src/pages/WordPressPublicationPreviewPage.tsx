...
export function WordPressPublicationPreviewPage() {
  ...
  return <section className="wordpress-page" aria-labelledby="wordpress-preview-title" aria-busy={preview.isPending}>
    <div className="page-heading-with-actions"><div><p className="dashboard__eyebrow">WordPress · Preview + draft</p><h1 id="wordpress-preview-title">Publication Dry Run</h1><p className="wordpress-page__description">DB source of truth에서 payload를 만들고 읽기 전용 검사를 마친 뒤, 명시적으로 확인한 경우에만 draft 1건을 생성합니다.</p></div><button className="primary-button wordpress-page__action" type="button" disabled={preview.isPending} onClick={() => preview.mutate()}>{preview.isPending ? 'Dry Run 실행 중' : 'Dry Run 실행'}</button></div>
    ...
    {preview.data ? <WordPressPublicationPlanPanel plan={preview.data} sourceTitle={post.data.title} stale={stale} client={supabase} userId={user?.id ?? ''} /> : null}
  </section>
}
