...
export function buildNewsBriefingPrompt(context: NewsBriefingPromptContext, mode: BriefingPromptMode): string {
  ...
  const sections: string[] = [
    ...
    '- 기사 문장을 복사하거나 해외 기사를 전체 직역하지 않고 독립적인 표현과 구성으로 재작성한다.',
    '- 언론사 이미지·방송 화면·도표를 무단 사용하거나 모사하지 않는다.',
    '- 뉴스 수를 억지로 채우지 않는다.', '',
    '[카테고리 조사 범위]',
  ...
  sections.push('[최근 종료 주제]', ...closedTopics(context, mode), '',
    ...
    '- 정정은 바뀐 사실과 이전 오류를, 종료 메모는 종료 근거를 명시한다. 변화가 없는 반복 보도는 포함하지 않는다.', '',
    '[최종 출력 순서]',
    '1. SEO 입력용 대표 제목',
    '2. SEO 대안 제목 4개',
    '3. 메타 설명',
  ...
  sections.push('[END_DAILY_BRIEF_NOTE_PROMPT]')
