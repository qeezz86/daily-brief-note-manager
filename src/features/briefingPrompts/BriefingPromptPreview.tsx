...
export function BriefingPromptPreview({ context, prompt, requestedPostCount, promptCopyDisabled = false }: { context: NewsBriefingPromptContext; prompt: string; requestedPostCount: BriefingPromptRecentCount; promptCopyDisabled?: boolean }) {
  ...
  return <div className="prompt-results">
    <section className="prompt-panel" aria-labelledby="prompt-counts"><h2 id="prompt-counts">집계 현황</h2><p>요청 {requestedPostCount}개 · 실제 사용 {counts.recentPosts}개</p><dl className="prompt-counts"><div><dt>최근 브리핑</dt><dd>{counts.recentPosts}</dd></div><div><dt>최근 뉴스 항목</dt><dd>{counts.recentUpdates}</dd></div><div><dt>추적 중 주제</dt><dd>{counts.openTopics}</dd></div><div><dt>pending 후속</dt><dd>{counts.pendingFollowups}</dd></div><div><dt>마감 초과</dt><dd>{counts.overdueFollowups}</dd></div><div><dt>최근 종료 주제</dt><dd>{counts.recentClosedTopics}</dd></div></dl></section>
