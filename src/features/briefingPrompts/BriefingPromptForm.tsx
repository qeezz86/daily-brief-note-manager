...
export function BriefingPromptForm({
  categories, value, disabled, onChange, onSubmit,
}: {
  categories: Category[]
  ...
  function submit(event: FormEvent) { event.preventDefault(); if (value.categoryId && validLookback) onSubmit() }
  return <form className="prompt-settings" onSubmit={submit}>
    ...
    <button className="primary-button" type="submit" disabled={disabled || !value.categoryId || !value.referenceDate || !validLookback}>{disabled ? '집계 중' : '프롬프트 생성'}</button>
  </form>
}
