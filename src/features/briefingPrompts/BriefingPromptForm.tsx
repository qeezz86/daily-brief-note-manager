...
import {
  ...
  type BriefingPromptRecentCount,
  type BriefingPromptSettings,
...
export function BriefingPromptForm({
  ...
  return <form className="prompt-settings" onSubmit={submit}>
    <fieldset className="post-form__section"><legend>설정</legend>
      ...
      <div className="post-form__field"><label htmlFor="prompt-recent-count">최근 글 수</label><select id="prompt-recent-count" value={value.recentPostCount ?? 5} onChange={(event) => onChange({ ...value, recentPostCount: Number(event.target.value) as BriefingPromptRecentCount })}>{briefingPromptRecentCounts.map((count) => <option key={count} value={count}>최근 글 {count}개</option>)}</select></div>
