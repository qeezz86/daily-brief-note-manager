...
describe('briefing prompt query keys', () => {
  ...
  it('includes all settings and recent count', () => expect(briefingPromptQueryKeys.context('owner', settings)).toEqual(['briefing-prompts', 'context', 'owner', 'economy', '2026-07-13', 'standard', 5, 90]))
  it('isolates different recent-count caches', () => expect(briefingPromptQueryKeys.context('owner', settings)).not.toEqual(briefingPromptQueryKeys.context('owner', { ...settings, recentPostCount: 15 })))
