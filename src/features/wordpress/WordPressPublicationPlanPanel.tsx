...
export function WordPressPublicationPlanPanel({ plan, sourceTitle, stale }: { plan: PublicationPlan; sourceTitle: string; stale: boolean }) {
  ...
  return <div className="wordpress-plan">
    ...
    {plan.blockers.length ? <section className="wordpress-panel wordpress-panel--warning" role="alert"><h2>Blockers</h2><ul>{plan.blockers.map((issue) => <li key={`${issue.code}-${issue.message}`}><code>{issue.code}</code> {issue.message}</li>)}</ul></section> : null}
    {plan.warnings.length ? <section className="wordpress-panel wordpress-panel--warning"><h2>Warnings</h2><ul>{plan.warnings.map((issue) => <li key={`${issue.code}-${issue.message}`}><code>{issue.code}</code> {issue.message}</li>)}</ul></section> : null}
