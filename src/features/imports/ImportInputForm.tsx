  ...
  return (
    <section className="import-panel" aria-labelledby="import-input-title">
      ...
      <div className="segmented-control import-mode" aria-label="입력 방식">
        <button type="button" disabled={disabled} aria-pressed={mode === 'file'} onClick={() => changeMode('file')}>JSON 파일</button>
        <button type="button" disabled={disabled} aria-pressed={mode === 'text'} onClick={() => changeMode('text')}>JSON text</button>
      </div>
      {mode === 'file' ? <label className="import-input-field">JSON 파일<input ref={fileInputRef} disabled={disabled} type="file" accept=".json,application/json" onChange={(event) => { setFile(event.target.files?.[0] ?? null); setError(null); onReset() }} />{file ? <small>{file.name} · {file.size.toLocaleString()} bytes</small> : null}</label> : <label className="import-input-field">JSON text<textarea disabled={disabled} rows={14} value={text} placeholder={'{\n  "format": "daily-brief-note-content-import",\n  "schemaVersion": 1,\n  "posts": []\n}'} onChange={(event) => { setText(event.target.value); setError(null); onReset() }} /></label>}
