...
export function ImportJobList({ jobs }: { jobs: ImportJobListItem[] }) {
  ...
  return <div className="import-job-list">{jobs.map((job) => <article className="import-job-card" key={job.id}>
    <div><p className="dashboard__eyebrow">{job.status}{job.restoredFromBackup ? ' · 복원된 과거 이력' : ''}</p><h2><Link to={`/imports/history/${job.id}`}>{job.sourceName ?? '이름 없는 JSON 입력'}</Link></h2><p>{job.sourceFingerprint.slice(0, 12)}… · {job.format} v{job.schemaVersion}</p>{job.executionLocked ? <p>실행 잠금 · origin {job.restoreOriginChecksum?.slice(0, 12) ?? '-'}…</p> : null}</div>
