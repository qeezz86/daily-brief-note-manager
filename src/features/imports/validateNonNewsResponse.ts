...
function labelValue(root: ParentNode, labels: string[]) {
  for (const node of Array.from(root.querySelectorAll('tr, li, p, div, dt, dd'))) {
    if (node.tagName === 'TR') {
      ...
      const key = normalizeHeadingText(cells[0]?.textContent ?? '').replace(/[:：]$/u, '')
      if (labels.includes(key) && cells[1]) return normalizeHeadingText(cells[1].textContent ?? '')
    }
    if (node.tagName === 'DT') {
      const key = normalizeHeadingText(node.textContent ?? '').replace(/[:：]$/u, '')
      const sibling = node.nextElementSibling
      if (labels.includes(key) && sibling?.tagName === 'DD') return normalizeHeadingText(sibling.textContent ?? '')
    }
...
function sourceFromAnchor(anchor: HTMLAnchorElement): NonNewsResponseSourceCandidate {
  ...
  return {
    ...
    sourceTitle: container.dataset.sourceTitle?.trim() ?? normalizeHeadingText(anchor.textContent ?? ''),
...
export function extractNonNewsResponseSources(document: Document): NonNewsResponseSourceCandidate[] {
  const candidates: NonNewsResponseSourceCandidate[] = []
  for (const section of Array.from(document.querySelectorAll<HTMLElement>('#sources, #source-check'))) {
    ...
    if (section.id === 'source-check') {
      ...
      candidates.push({
        ...
        sourceTitle: labelValue(section, ['원문 제목', '출처 제목']) || normalizeHeadingText(anchor?.textContent ?? ''),
  ...
  const unique = new Map<string, NonNewsResponseSourceCandidate>()
...
function inspectElementTree(root: ParentNode, issues: NonNewsResponseIssue[]) {
  prohibitedElements.forEach((tag) => {
    if (root.querySelector(tag)) issues.push(issue('NON_NEWS_HTML_EXECUTABLE_CONTENT', 'invalid', `허용되지 않은 <${tag}> 요소가 포함되어 있습니다.`, 'wordpressHtml'))
  })
  const elements = Array.from(root.querySelectorAll<HTMLElement>('*'))
  elements.forEach((element) => {
    for (const attribute of Array.from(element.attributes)) {
      const name = attribute.name.toLocaleLowerCase('en-US')
      if (name.startsWith('on') || prohibitedAttributes.has(name)) {
        issues.push(issue('NON_NEWS_HTML_EXECUTABLE_CONTENT', 'invalid', `허용되지 않은 ${attribute.name} 속성이 포함되어 있습니다.`, 'wordpressHtml'))
      }
      if (urlAttributes.has(name) && activeSchemePattern.test(attribute.value)) {
        issues.push(issue('NON_NEWS_HTML_ACTIVE_URL_SCHEME', 'invalid', 'javascript:, data:, vbscript: URL은 사용할 수 없습니다.', 'wordpressHtml'))
      }
    }
    if (element instanceof HTMLTemplateElement) inspectElementTree(element.content, issues)
  })
}
