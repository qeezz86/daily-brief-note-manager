...
export function ImportExecutionProgress({ progress }: { progress: ImportProgressState }) {
  ...
  return <section className="import-panel" aria-labelledby="import-progress-title">
    ...
    <div className="import-progress-counts"><span>성공 {progress.imported}</span><span>실패 {progress.failed}</span><span>건너뜀 {progress.skipped}</span><span>남음 {remaining}</span></div>
  </section>
}
