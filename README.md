...
현재 저장소는 Phase 5C 단계입니다. 인증된 단일 허용 사용자는 콘텐츠별 publication payload를 Dry Run으로 검토한 뒤 명시적 확인을 거쳐 WordPress `draft` 1건을 생성할 수 있습니다. 서버는 쓰기 직전에 RLS 범위의 DB 원본, taxonomy, duplicate slug, source `updated_at`과 canonical fingerprint를 다시 검증합니다. WordPress credential은 브라우저나 DB에 저장하지 않으며 publish·update·delete·media·taxonomy write와 자동 재시도는 수행하지 않습니다. 자세한 보안 경계는 [`docs/WORDPRESS_DRAFT_CREATION.md`](docs/WORDPRESS_DRAFT_CREATION.md)를 참고합니다.
