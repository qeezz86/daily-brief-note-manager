...
`/imports`는 UTF-8 `.json` 파일과 직접 붙여넣은 JSON 중 마지막으로 선택한 한 입력만 사용합니다. 파일 제한은 20 MB, 게시물 제한은 2,000개이며 BOM, 과도한 중첩·문자열, prototype pollution 키를 차단합니다. 외부 JSON은 camelCase와 strict 최상위 schema를 사용하며 format 없는 legacy bundle, 다른 format, 지원하지 않는 schema version을 허용하지 않습니다. `validationMode` 기본값은 `strict`이고 기존 WordPress 글은 명시적으로 `legacy`를 사용할 수 있지만 script, iframe, event handler와 `javascript:` URL 같은 보안 오류는 두 모드 모두 차단합니다. DB exact duplicate 후보는 trim·빈 값 제외·중복 제거 후 100개씩 RLS 범위의 순차 batch query로 확인합니다. 조회 상태는 `complete`, `partial`, `unavailable`이며 실제 Import는 `complete`에서만 허용합니다. 입력·Dry Run·카테고리 설정이 바뀌면 선택과 warning 승인은 초기화되거나 stale 처리됩니다. 실행 결과는 현재 브라우저 세션에만 남아 새로고침하면 사라질 수 있습니다. 전체 백업의 `data` schema 또는 backup format은 콘텐츠 Import bundle과 다르므로 이 화면에서 받지 않습니다.
